...
precompute_and_store_diffExp_and_foldchange_matrices <- function(folder = "/tmp"){
  ...
  v.colnames_mandatory = c("sample_id", "series_id", "condition_treatment_1", "condition_treatment_2", "condition_tissue","replicate_group_w_control", "sublevel_series_id", "superlevel_series_id")
  ...
  # saveRDS(m.directionality, paste(folder, "/m.directionality.rds", sep = ""))
  # saveRDS(m.de, paste(folder, "/m.de.rds", sep = ""))
  # saveRDS(m.fc, paste(folder, "/m.fc.rds", sep = ""))
  # saveRDS(v.series_sets, paste(folder, "/v.series_sets.rds", sep = ""))
  # saveRDS(v.repeats, paste(folder, "/v.repeats.rds", sep = ""))
  # saveRDS(v.treatment_buildingblocks, paste(folder, "/v.treatments.rds", sep = ""))
  # 
  # 
  # 
  # 
  # 
  # m.de = readRDS("tmp/m.de.rds")
  # m.fc = readRDS("tmp/m.fc.rds")
  # 
  # 
  ...
  df.annotation_series = unique(df.annotation[,c("series_id","condition_treatment_1", "condition_treatment_2", "condition_tissue")])
  df.annotation_series["number_series"] = table(v.series_sets)[df.annotation_series$series_id]
  write.table(df.annotation_series, "A:/junkDNA.ai/datasets/experiment_series_annotation.txt", row.names = F, sep = "\t")
  
  genes = rownames(m.de)
  experiment_series_id = colnames(m.de)
  write.table(genes, "A:/junkDNA.ai/datasets/genes.txt", row.names = F, col.names = F, sep = "\t")
  write.table(experiment_series_id, "A:/junkDNA.ai/datasets/experiment_series_id.txt", row.names = F, col.names = F, sep = "\t")
